...
void fill_dma_desc(volatile lldesc_t* dmadesc, i2s_parallel_buffer_desc_t* bufdesc) {
	int n = 0;
...
static void gpio_setup_out(int gpio, int sig) {
	if (gpio < 0 || gpio >= GPIO_PIN_COUNT) return;
	PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[gpio], PIN_FUNC_GPIO);
	gpio_set_direction(gpio, GPIO_MODE_DEF_OUTPUT);
	gpio_matrix_out(gpio, sig, false, false);
//	gpio_set_drive_capability(gpio, GPIO_DRIVE_CAP_MAX);
	}
...
void i2s_parallel_setup_without_malloc(i2s_dev_t* dev, const i2s_parallel_config_t* cfg) {
	...
	rtc_clk_apll_enable(1, 0, 0, 5, 0);		// enable, sdm0 = 0, sdm1 = 73, sdm2 = 7, odir = 2 ->50Mhz
	...
	dev->clkm_conf.clkm_div_num = 50000000L / (cfg->clkspeed_hz);

	// set_clock_divs(dev, cfg->clkspeed_hz);
...
void set_clock_divs(i2s_dev_t* dev, uint32_t Clock)
	{
	uint32_t Div, A, B, LastDiv = 1, LastA = 63, LastB = 63, LastClock = Clock, Diff;
	float Res;
	bool Done = false;
	...
	for (Div = 2; Div < 256; Div++) {
		for (A = 63; A; A--) {
			for (B = 63; B; B--) {
				Res = 80000000.0 / (float)((float)Div + (float)B / (float)A);
				if (Res == Clock) {
					Done = true;
					break;
					}
				else {
					Diff = labs((int32_t)Res - Clock);
					if (Diff < LastClock) {
						LastClock = Diff; LastA = A; LastB = B; LastDiv = Div;
						}
					}
				}
			if (Done)
	...
	if (!Done) {
		A = LastA; B = LastB; Div = LastDiv;
		Res = 80000000.0 / (float)((float)Div + (float)B / (float)A);
		}
	printf("Freq=%f hz, Div:%u, A:%u, B:%u\n", Res, Div, A, B);

	dev->clkm_conf.val = 0;
	dev->clkm_conf.clkm_div_a = A;
	dev->clkm_conf.clkm_div_b = B;
	dev->clkm_conf.clkm_div_num = Div;
	}
